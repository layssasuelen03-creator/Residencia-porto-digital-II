document.addEventListener("DOMContentLoaded", function() {
  ...
  const tecidoCards = Array.from(document.querySelectorAll(".modelagem-card"));

  // === 1️⃣ BOTÃO ADICIONAR - REDIRECIONA PARA CADASTRO ===
  if (btnAdicionar) {
    btnAdicionar.addEventListener("click", () => {
      window.location.href = "cadastro-modelagens.html";
    });
  }
  ...
  menuButtons.forEach(btn => {
    btn.addEventListener("click", (event) => {
      ...
      btnEditar.addEventListener("click", () => {
        window.location.href = "editar-modelagens.html";
      });
